...
class CPEconv(nn.Module):
    def __init__(self, in_channels, spatial_shape, kernel_size=(3, 3, 3), padding=(1, 1, 1)):
        ...
        # self.conv3d = nn.Conv3d(in_channels, in_channels, kernel_size=kernel_size, padding=padding)
        ...
        total_params = sum(p.numel() for p in self.conv3d.parameters())
        print(f"Total cpe parameters: {total_params}")
...
class Attention(nn.Module):
    def __init__(
        ...
        total_params = sum(p.numel() for p in self.rpe.parameters())
        print(f"Total rpe parameters: {total_params}")
        # import pdb;pdb.set_trace()
