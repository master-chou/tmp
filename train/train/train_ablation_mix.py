...
class DepthSpatialDataset2(Dataset):
    def __init__(self, root_dir='', transform=None, depth_transform=None):
        self.root_dir = root_dir
        self.transform = transform
        self.depth_transform = depth_transform
    ...
    def __getitem__(self, idx):
        ...
        record = self.metadata[idx]
        img_name = record['image_id']
        spatial_caption = record['caption'].get('coca_caption')
        image_path=f'/dataset/SA-1B/data/sa_{record["image_id"][3:-8]:0>6}/{record["image_id"]}'
        # image_path = os.path.join(self.root_dir, img_name + '.jpg')
        depth_path = f'/home/aiops/wangzh/data/depth-image/{record["image_id"]}'
...
class CLIP_Clean_Train():
    ...
    def train(self, common_pair=False, resume=False, amp=False, warmup_length=200):
        ...
        trainset_spat = DepthSpatialDataset( transform=clip_transform, depth_transform=depth_transform)
        trainset_coca = DepthSpatialDataset2( transform=clip_transform, depth_transform=depth_transform)
        trainset_gpt = DepthDataset( transform=clip_transform, depth_transform=depth_transform)
        
        trainset=ConcatDataset([trainset_spat,trainset_coca,trainset_gpt])
