import torch
import alpha_clip_new as alpha_clip
import os
import sys
import wandb
# 获取上级目录的绝对路径
...
CC3M_ROOT = '/home/aiops/wangzh/zza/data/LLaVA-ReCap-CC3M/data/'
# CC3M_CAPS = '/home/aiops/wangzh/zza/merge_caps.parquet'
CC3M_CAPS = '/home/aiops/wangzh/zza/merge_part_caps.parquet'
CC3M_IMG_ROOT = '/home/aiops/wangzh/zza/data/LLaVA-ReCap-CC3M/imgs/'
IN1K_ROOT = '/home/aiops/wangzh/zza/data/imagenet-1k/data/train/'
DEPTH_ROOT = '/home/aiops/wangzh/data/cc3m-depth'
...
class cc3m_train_dataset(Dataset):
    def __init__(self,clip_transform, depth_transform):
        # self.cc3m_root = CC3M_ROOT
        # self.origin_captions = pd.read_parquet(CC3M_CAPS)
        self.clip_transform = clip_transform
        # self.cc3m_data = [pd.read_parquet(CC3M_ROOT + 'train-{0:05}-of-00281.parquet'.format(i)) for i in range(281)]
        # self.cc3m_data = pd.concat(self.cc3m_data, ignore_index=True)
        # self.cc3m_data = cc3m_data
        self.depth_transform=depth_transform
        self.meta = pd.read_parquet(CC3M_CAPS)
        # if clip_dim == 512:
        #     self.clip_image_embs = np.load(CLIP_B16_CC3M_TRAIN_IMAGE, mmap_mode='r')
        #     self.clip_text_embs  = np.load(CLIP_B16_CC3M_TRAIN_TEXT , mmap_mode='r')
        # else:
        #     self.clip_image_embs = np.load(CLIP_L336_CC3M_TRAIN_IMAGE, mmap_mode='r')
        #     self.clip_text_embs  = np.load(CLIP_L336_CC3M_TRAIN_TEXT , mmap_mode='r')
    ...
    def __getitem__(self, index):
        # caption = self.meta['conversations'][index][1]['value']
        caption_short = self.meta['origin_caption'][index]
        # use_short = random.random() < self.short_ratio
        # if use_short:
        #     caption = caption_short
        # img_index = self.meta['abs_id'][index]
        # image = Image.open(BytesIO(self.cc3m_data['image'][img_index]['bytes'])).convert('RGB')
        # image_inputs = self.preprocess(images = image, return_tensors='pt')
        image_path = os.path.join(CC3M_IMG_ROOT, '{0:05}'.format(self.meta['part'][index]), self.meta['ids'][index] + '.jpg')
        image = Image.open(image_path).convert('RGB')
        # image_inputs = self.preprocess(images = image, return_tensors='pt')
        image_input=self.clip_transform(image)
        depth_path = os.path.join(DEPTH_ROOT, '{0:05}'.format(self.meta['part'][index]), self.meta['ids'][index] + '.jpg')
        try:
            depth = Image.open(depth_path).convert('L')
            if self.depth_transform:
                depth = self.depth_transform(depth)
            ...
            print(f"Warning: Failed to load depth image {depth_path}. Error: {e}")
        # clip_image_emb = torch.from_numpy(self.clip_image_embs[index])
        # clip_text_emb  = torch.from_numpy(self.clip_text_embs[index])
        # print(caption)
        if image_input is None or depth is None:
            return self.__getitem__((idx + 1) % len(self))  # 返回下一个有效的数据点
        return image_input, depth, caption_short
...
class CLIP_Clean_Train():
    def __init__(self, local_rank=0, lr=4e-5, weigth_decay=0.02, log_scale=4.6052, lora_rank=-1, common_pair=0.0, para_gamma=0.01, exp_name="auto", warmup_length=200, epoch_num=1, subnum=10000,pos_embed=None):
        ...
        if lora_rank == -1:
            ...
            self.model, _ = alpha_clip.load("ViT-L/14@336px", device='cpu', lora_adapt=True, rank=lora_rank)
        ...
        if lora_rank != -1: # use lora
            ...
            for k, v in self.model.named_parameters():
                ...
                elif ("transformer" not in k) and ("visual" in k):
                    v.requires_grad_(True)
                    other_opt_paras.append(v)
        # if lora_rank != -1: # use lora
        #     lora.mark_only_lora_as_trainable(self.model)
    ...
    def test_epoch_scannet(self, dataloader):
        ...
        # self.text_embeddings = zeroshot_classifier_ours(testset.captions, self.model, self.local_rank)
        sampler = DistributedSampler(dataset=testset, shuffle=False)
        dataloader = torch.utils.data.DataLoader(testset, batch_size=2000, sampler=sampler, num_workers=0)
    ...
    def train(self, common_pair=False, resume=False, amp=False, warmup_length=200):
        ...
        # test_sampler = DistributedSampler(dataset=testset, shuffle=True)
        # test_loader = torch.utils.data.DataLoader(testset, batch_size=self.batch_size, sampler=test_sampler, num_workers=16, pin_memory=True)
        for name, testset in zip(['COCO'], [coco_dataset]):
            # test_sampler = DistributedSampler(dataset=testset, shuffle=True)
        ...
        # model, clip_preprocess = clip.load("ViT-B/16")
        # trainset = cc3m_train_dataset(clip_transform,depth_transform)
        # import pdb;pdb.set_trace()
...
if __name__ == "__main__":
    ...
    # acc=trainer.test_epoch_scannet(None)
    wandb.finish()
