    import torch
import alpha_clip_full3 as alpha_clip
import os
import sys
import wandb
# 获取上级目录的绝对路径
...
class DepthSpatialDataset(Dataset):
    def __init__(self, root_dir='', transform=None, depth_transform=None):
        ...
        json_file_path='/home/aiops/wangzh/data/final_metadata_all_new.json'
        neg_file_path='/home/aiops/wangzh/data/final_metadata_all_negative.json'
    ...
    def __getitem__(self, idx):
        ...
        gpt_caption = record.get('caption-gpt','no this caption')
        coca_caption = record['caption'].get('coca_caption','no this caption')
        spatial_caption = record['caption'].get('spatial_caption','no this caption')
        neg_record = self.negdata[idx]
        neg_gpt_caption = neg_record.get('caption-gpt','no this caption')
        neg_coca_caption = neg_record['caption'].get('coca_caption','no this caption')
        neg_spatial_caption = neg_record['caption'].get('spatial_caption','no this caption')
        ...
        return image, depth, posi_caption, neg_caption
...
class CLIP_Clean_Train():
    ...
    def inference(self, images, masks, posi_texts,neg_texts):
        # import pdb;pdb.set_trace()
    ...
    def train_epoch(self, dataloader, test_loaders, epoch, start_iter=0, amp=False):
        ...
        for i, (images, masks, posi_texts,neg_texts) in enumerate(tqdm(dataloader, disable=(dist.get_rank() != 0))):
            # import pdb;pdb.set_trace()
            # print(texts)
            ...
            if amp:
                with torch.cuda.amp.autocast():
                    loss = self.inference(images, masks, posi_texts,neg_texts)
